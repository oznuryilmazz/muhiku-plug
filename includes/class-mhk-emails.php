<?php
/**
 * This class handles all (notification) emails sent by Muhiku Plug.
 *
 ...
 *
 * @package MuhikuPlug\Classes\Emails
 * @version 1.2.0
 * @since   1.0.0
 */
...
class MHK_Emails {
	...
	public function process_tag( $string = '', $sanitize = true, $linebreaks = false ) {
		...
		if ( $sanitize ) {
			if ( $linebreaks ) {
				$tag = mhk_sanitize_textarea_field( $tag );
			} else {
	...
	public function muhiku_forms_html_field_value( $html = true ) {
		...
		if ( $html ) {
			...
			$empty_message  = '<em>' . __( '(empty)', 'muhiku-plug' ) . '</em>';
			...
			foreach ( $this->fields as $meta_id => $field ) {
				...
				if ( empty( $field_name ) ) {
					$field_name = sprintf(
						...
						esc_html__( 'Field ID #%d', 'muhiku-plug' ),
						absint( $field['id'] )
					);
				}
			...
			foreach ( $this->fields as $field ) {
				...
				$field_val  = empty( $field['value'] ) && '0' !== $field['value'] ? esc_html__( '(empty)', 'muhiku-plug' ) : $field['value'];
		...
		if ( empty( $message ) ) {
			$empty_message = esc_html__( 'An empty form was submitted.', 'muhiku-plug' );
