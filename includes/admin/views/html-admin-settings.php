...
$tab_exists        = isset( $tabs[ $current_tab ] ) || has_action( 'muhiku_forms_sections_' . $current_tab ) || has_action( 'muhiku_forms_settings_' . $current_tab );
...
<div class="wrap muhiku-plug">
	...
	<form method="<?php echo esc_attr( apply_filters( 'muhiku_forms_settings_form_method_tab_' . $current_tab, 'post' ) ); ?>" id="mainform" action="" enctype="multipart/form-data">
	<?php endif; ?>
		<nav class="nav-tab-wrapper mhk-nav-tab-wrapper">
			...
			do_action( 'muhiku_forms_settings_tabs' );
			?>
		</nav>
		<div class="muhiku-plug-settings">
			<h1 class="screen-reader-text"><?php echo esc_html( $current_tab_label ); ?></h1>
			<?php
				do_action( 'muhiku_forms_sections_' . $current_tab );

				self::show_messages();

				do_action( 'muhiku_forms_settings_' . $current_tab );
			?>
			<p class="submit">
