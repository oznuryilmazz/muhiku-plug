...
class MHK_Settings_reCAPTCHA extends MHK_Settings_Page {
	...
	public function __construct() {
		$this->id    = 'recaptcha';
		$this->label = esc_html__( 'CAPTCHA', 'muhiku-plug' );
	...
	public function get_settings() {
		...
		foreach ( $languages['languages'] as $key => $value ) {
			...
			$lang_options[ $value['Value'] ] = sprintf( esc_html__( '%s', 'muhiku-plug' ), $value['Language'] ); // phpcs:ignore
		}
		...
		$settings = apply_filters(
			...
			array(
				...
				array(
					...
					'type'     => 'radio',
					'options'  => array(
						'v2'       => esc_html__( 'reCAPTCHA v2', 'muhiku-plug' ),
						'v3'       => esc_html__( 'reCAPTCHA v3', 'muhiku-plug' ),
						'hcaptcha' => esc_html__( 'hCaptcha', 'muhiku-plug' ),
					),
					'class'    => 'muhiku-plug-recaptcha-type',
					'desc_tip' => true,
				),
				array(
				...
				array(
					'title'             => esc_html__( 'Threshold Score', 'muhiku-plug' ),
					'type'              => 'number',
					/* translators: %1$s - Google reCAPTCHA docs url */
