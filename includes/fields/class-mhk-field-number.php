 ...
 */
class MHK_Field_Number extends MHK_Form_Fields {

	/**
	 * Constructor.
	 */
	public function __construct() {
		$this->name     = esc_html__( 'Number', 'muhiku-plug' );
		$this->type     = 'number';
		$this->icon     = 'mhk-icon mhk-icon-number';
		$this->order    = 80;
		$this->group    = 'general';
		$this->settings = array(
			'basic-options'    => array(
	...
	public function step( $field ) {
		$label       = $this->field_element(
			...
			array(
				'slug'    => 'step',
				'value'   => esc_html__( 'Step', 'muhiku-plug' ),
				'tooltip' => esc_html__( 'Allows users to enter specific legal number intervals.', 'muhiku-plug' ),
			),
			false
		);
	...
	public function min_value( $field ) {
		$label       = $this->field_element(
			...
			array(
				'slug'    => 'min_value',
				'value'   => esc_html__( 'Min Value', 'muhiku-plug' ),
				'tooltip' => esc_html__( 'Minimum value user is allowed to enter.', 'muhiku-plug' ),
			),
			false
		);
		$input_field = $this->field_element(
	...
	public function max_value( $field ) {
		$label       = $this->field_element(
			...
			array(
				'slug'    => 'max_value',
				'value'   => esc_html__( 'Max Value', 'muhiku-plug' ),
				'tooltip' => esc_html__( 'Maximum value user is allowed to enter.', 'muhiku-plug' ),
			),
			false
		);
		$input_field = $this->field_element(
	...
	public function validate( $field_id, $field_submit, $form_data ) {
		...
		if ( ! empty( $form_data['form_fields'][ $field_id ]['required'] ) && '1' !== $conditional_status && empty( $field_submit ) && '0' !== $field_submit ) {
			mhk()->task->errors[ $form_id ][ $field_id ] = mhk_get_required_label();
		...
		if ( ! empty( $form_data['form_fields'][ $field_id ]['min_value'] ) && ! empty( $field_submit ) && floatval( $field_submit ) < $min_value ) {
			...
			mhk()->task->errors[ $form_id ][ $field_id ] = sprintf( esc_html__( 'Please enter a value greater than or equal to %s', 'muhiku-plug' ), absint( $min_value ) );
			update_option( 'mhk_validation_error', 'yes' );
		} elseif ( ! empty( $form_data['form_fields'][ $field_id ]['max_value'] ) && ! empty( $field_submit ) && floatval( $field_submit ) > $max_value ) {
			/* translators: %s - maximum value. */
			mhk()->task->errors[ $form_id ][ $field_id ] = sprintf( esc_html__( 'Please enter a value less than or equal to %s', 'muhiku-plug' ), absint( $max_value ) );
			update_option( 'mhk_validation_error', 'yes' );
		}
	}
}
