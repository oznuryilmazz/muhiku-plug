...
function mhk_get_entry( $id, $with_fields = false, $args = array() ) {
	...
	$entry = wp_cache_get( $id, 'mhk-entry' );
	if ( false === $entry ) {
		$entry = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}mhk_entries WHERE entry_id = %d LIMIT 1;", $id ) ); // WPCS: cache ok, DB call ok.
	...
	$fields = mhk_decode( $entry->fields );
	...
	if ( $with_fields && ! empty( $fields ) ) {
		...
		if ( false === $results ) {
			$results = $wpdb->get_results( $wpdb->prepare( "SELECT meta_key, meta_value FROM {$wpdb->prefix}mhk_entrymeta WHERE entry_id = %d", $id ), ARRAY_A );
			wp_cache_add( $id, $results, 'mhk-entrymeta' );
		}
 ...
 * @return int[]
 */
function mhk_get_entries_ids( $form_id ) {
	global $wpdb;

	$results = wp_cache_get( $form_id, 'mhk-entries-ids' );
	if ( false === $results ) {
		$results = $wpdb->get_results( $wpdb->prepare( "SELECT entry_id FROM {$wpdb->prefix}mhk_entries WHERE form_id = %d", $form_id ) ); // WPCS: cache ok, DB call ok.
		wp_cache_add( $form_id, $results, 'mhk-entries-ids' );
	}
 ...
 * @return array
 */
function mhk_search_entries( $args ) {
	global $wpdb;
 ...
 * @return array
 */
function mhk_get_count_entries_by_status( $form_id ) {
	$form_data = mhk()->form->get( $form_id, array( 'content_only' => true ) );
	$statuses  = array_keys( mhk_get_entry_statuses( $form_data ) );
	$counts    = array();
 ...
 * @return int[]
 */
function mhk_get_count_entries_by_last_entry( $form_id, $last_entry ) {
	global $wpdb;

	$results = wp_cache_get( $form_id, 'mhk-last-entries-count' );

	if ( false === $results ) {
		$results = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(entry_id) FROM {$wpdb->prefix}mhk_entries WHERE form_id = %d AND entry_id > %d", $form_id, $last_entry ) );
		wp_cache_add( $form_id, $results, 'mhk-last-entries-count' );
	}
 ...
 */
function mhk_get_entries_by_form_id( $form_id, $start_date = '', $end_date = '' ) {
	global $wpdb;

	$query   = array();
	$query[] = $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}mhk_entries WHERE form_id=%s", $form_id );
