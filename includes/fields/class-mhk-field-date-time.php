...
class MHK_Field_Date_Time extends MHK_Form_Fields {
	...
	public function __construct() {
		$this->name     = esc_html__( 'Date / Time', 'muhiku-plug' );
		$this->type     = 'date-time';
		$this->icon     = 'mhk-icon mhk-icon-calendar';
		$this->order    = 20;
		$this->group    = 'advanced';
		$this->settings = array(
			'basic-options'    => array(
	...
	public function choose_format( $field ) {
		...
		$format_label  = $this->field_element(
			...
			array(
				...
				'value'   => esc_html__( 'Format', 'muhiku-plug' ),
				'tooltip' => esc_html__( 'Select a format for the date field.', 'muhiku-plug' ),
			),
			false
		);
		$format_select = $this->field_element(
			...
			array(
				...
				'options' => array(
					...
					'time'      => esc_html__( 'Time', 'muhiku-plug' ),
					'date-time' => esc_html__( 'Both', 'muhiku-plug' ),
				),
			),
			false
		);
	...
	public function choose_style( $field ) {
		...
		$style_label  = $this->field_element(
			...
			array(
				...
				'tooltip' => esc_html__( 'Select a style for the date field.', 'muhiku-plug' ),
			),
			false
		);
		$style_select = $this->field_element(
			...
			array(
				...
				'value'   => $style,
				'options' => array(
					'picker'   => esc_html__( 'Date Picker', 'muhiku-plug' ),
					'dropdown' => esc_html__( 'Date Dropdown', 'muhiku-plug' ),
				),
			),
			false
		);
	...
	public function datetime_options( $field ) {
		$format             = ! empty( $field['datetime_format'] ) ? esc_attr( $field['datetime_format'] ) : 'date';
		$class_name         = isset( $field['enable_min_max'] ) && '1' === $field['enable_min_max'] ? '' : 'muhiku-plug-hidden';
		...
		$this->field_element(
			...
			array(
				...
				'value'   => esc_html__( 'Date & Time Format', 'muhiku-plug' ),
				'tooltip' => esc_html__( 'Advanced date and time formatting options.', 'muhiku-plug' ),
			),
			true
		);

		echo '<div class="format-selected-' . esc_attr( $format ) . ' format-selected">';
			echo '<div class="muhiku-plug-border-container muhiku-plug-date">';
			echo '<h4 class="muhiku-plug-border-container-title">' . esc_html__( 'Date', 'muhiku-plug' ) . '</h4>'; // phpcs:ignore WordPress.Security.NonceVerification
			...
			$date_format_label = $this->field_element(
				...
				array(
					'slug'    => 'date_format',
					'value'   => esc_html__( 'Date Format', 'muhiku-plug' ),
					'tooltip' => esc_html__( 'Choose a desire date format to display.', 'muhiku-plug' ),
				),
				false
			);
			...
			$clear_disabled_dates_button = sprintf( '<a href="#" class="mhk-clear-disabled-dates after-label-description">%s</a>', esc_html__( 'Clear', 'muhiku-plug' ) );
			$disable_dates_label         = $this->field_element(
				'label',
				$field,
				array(
					'slug'          => 'disable_dates',
					'value'         => esc_html__( 'Disable Dates', 'muhiku-plug' ),
					'tooltip'       => esc_html__( 'Select which dates you want to disable.', 'muhiku-plug' ),
			...
			$disable_dates               = $this->field_element(
				...
				array(
					...
					'class' => 'muhiku-plug-disable-dates',
					'data'  => array(
			...
			$current_date_mode = $this->field_element(
				...
				array(
					...
					'desc'    => esc_html__( 'Date Mode', 'muhiku-plug' ),
					'tooltip' => esc_html__( 'Select your desire date mode.', 'muhiku-plug' ),
					'options' => array(
						'single'   => 'Single',
						'range'    => 'Range',
						'multiple' => 'Multiple',
			...
			$date_localization_label = $this->field_element(
				...
				array(
					...
					'value'   => esc_html__( 'Date Localization', 'muhiku-plug' ),
					'tooltip' => esc_html__( 'Choose a desire date localization to display.', 'muhiku-plug' ),
				),
				false
			);
			...
			$current_date_default = $this->field_element(
				...
				array(
					...
					'desc'    => esc_html__( 'Default to current date.', 'muhiku-plug' ),
					'tooltip' => esc_html__( 'Check this option to set current date as default.', 'muhiku-plug' ),
				),
				false
			);
			...
			$enable_min_max = $this->field_element(
				...
				array(
					...
					'desc'    => esc_html__( 'Enable Min Max date.', 'muhiku-plug' ),
					'tooltip' => esc_html__( 'Check this option to set min max date.', 'muhiku-plug' ),
				),
				false
			);
			...
			$min_date_label = $this->field_element(
				...
				array(
					'slug'    => 'min_date',
					'value'   => esc_html__( 'Minimum Date', 'muhiku-plug' ),
					'tooltip' => esc_html__( 'Select minium date.', 'muhiku-plug' ),
				),
				false
			);
			...
			$max_date_label = $this->field_element(
				...
				array(
					'slug'    => 'max_date',
					'value'   => esc_html__( 'Maximum Date', 'muhiku-plug' ),
					'tooltip' => esc_html__( 'Select maximum date.', 'muhiku-plug' ),
				),
				false
			);
			...
			$args = array(
				'slug'    => 'date_format',
				'content' => $date_format_label . $date_format_select . $disable_dates_label . $disable_dates . $date_localization_label . $date_localization_select . '<div class="muhiku-plug-checklist muhiku-plug-checklist-inline">' . $current_date_mode . '</div><div class="muhiku-plug-current-date-format">' . $current_date_default . '</div><div class="muhiku-plug-min-max-date-format">' . $enable_min_max . '</div><div class="muhiku-plug-min-max-date-option ' . $class_name . '">' . $min_date_label . $min_date . $max_date_label . $max_date . '</div>',
			);
			$this->field_element( 'row', $field, $args );

			echo '</div>';

			echo '<div class="muhiku-plug-border-container muhiku-plug-time">';
			echo '<h4 class="muhiku-plug-border-container-title">' . esc_html__( 'Time', 'muhiku-plug' ) . '</h4>'; // phpcs:ignore WordPress.Security.NonceVerification
			...
			$time_format_label = $this->field_element(
				...
				array(
					'slug'    => 'time_interval',
					'value'   => esc_html__( 'Time interval and format', 'muhiku-plug' ),
					'tooltip' => esc_html__( 'Choose time interval and format to display.', 'muhiku-plug' ),
				),
				false
			);
			...
			$time_interval_select .= $this->field_element(
				...
				array(
					...
					'class'   => 'time_interval',
					'options' => array(
						'15' => esc_html__( '15 minutes', 'muhiku-plug' ),
						'30' => esc_html__( '30 minutes', 'muhiku-plug' ),
					),
				),
				false
			);
			$time_format_select    = $this->field_element(
				...
				array(
					...
					'class'   => 'time_format',
					'options' => array(
						'g:i A' => esc_html__( '12 H', 'muhiku-plug' ),
						'H:i'   => esc_html__( '24 H', 'muhiku-plug' ),
					),
				),
				false
			);
		...
		$enable_min_max_time .= $this->field_element(
			...
			array(
				...
				'desc'    => esc_html__( 'Enable Min Max Time.', 'muhiku-plug' ),
				'tooltip' => esc_html__( 'Check this option to set min max time.', 'muhiku-plug' ),
			),
			false
		);
		...
		$select_min_time = $this->field_element(
			...
			array(
				...
				'value'   => esc_html__( 'Minimum Time', 'muhiku-plug' ),
				'tooltip' => esc_html__( 'Select minium time.', 'muhiku-plug' ),
			),
			false
		);
		...
		$select_max_time = $this->field_element(
			...
			array(
				...
				'value'   => esc_html__( 'Maximum Time', 'muhiku-plug' ),
				'tooltip' => esc_html__( 'Select maximum time.', 'muhiku-plug' ),
			),
			false
		);
	...
	public function field_display( $field, $field_atts, $form_data ) {
		...
		if ( ! array_key_exists( 'datetime_style', $field ) || 'picker' === $field['datetime_style'] ) {
			...
			printf(
				'<input type="text" %s %s >',
				mhk_html_attributes( $primary['id'], $class, $primary['data'], $primary['attr'] ),
			...
			if ( 'date-time' === $field['datetime_format'] || 'date' === $field['datetime_format'] ) {
				...
				printf(
					'<select value="%s" %s >',
					esc_attr( gmdate( 'Y' ) ),
					mhk_html_attributes( 'year-select-' . esc_attr( $primary['id'] ) )
				);
				...
				printf(
					'<select value="%s" %s >',
					esc_attr( gmdate( 'm' ) ),
					mhk_html_attributes( 'month-select-' . esc_attr( $primary['id'] ) )
				);
				// Build the select options.
				for ( $i = 1; $i <= 12; $i++ ) {
					$month = ( $i < 10 ) ? '0' . $i : $i;
				...
				printf(
					'<select value="%s" %s >',
					esc_attr( gmdate( 'd' ) ),
					mhk_html_attributes( 'day-select-' . esc_attr( $primary['id'] ) )
				);
				// Build the select options.
				for ( $i = 1; $i <= 32; $i++ ) {
					$day = $i < 10 ? '0' . $i : $i;
			...
			if ( 'time' === $field['datetime_format'] || 'date-time' === $field['datetime_format'] ) {
				...
				printf(
					...
					mhk_html_attributes( 'hour-select-' . esc_attr( $primary['id'] ) )
				);
				...
				printf(
					'<select %s>',
					mhk_html_attributes( 'minute-select-' . esc_attr( $primary['id'] ) )
				);
				echo '</select>';
			}
